...
function SceneCard({ sceneId, label }: { sceneId: (typeof SCENE_IDS)[number]; label: string }) {
  const src = useSceneThumbnail(sceneId)
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/30 backdrop-blur-xl">
      ...
      <div className="px-4 py-3">
        <span className="font-sans text-caption font-[700] text-white/85">{label}</span>
      </div>
    </div>
  )
}

function UploadedCard({ image }: { image: GalleryImage }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/30 backdrop-blur-xl">
...
export default function AdminGalleryPanel() {
  const t = useT()
  const { images, refetch } = useGalleryImages()
    ...
    <div className="flex flex-col gap-12">
      <div>
        <div className="mb-6 flex items-center justify-between gap-4">
          <div>
            <h2 className="font-display text-h2 font-[300] text-white/95">{t.admin.gallery.uploadedTitle}</h2>
            <p className="font-sans text-caption text-mist">{t.admin.gallery.uploadedSubtitle}</p>
          </div>
        ...
        {images.length === 0 ? (
          <p className="font-sans text-caption text-mist">{t.admin.gallery.empty}</p>
        ) : (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
            {images.map((image) => (
              <UploadedCard key={image.id} image={image} />
            ))}
          </div>
        )}
      </div>
      ...
      <div>
        <h2 className="mb-1 font-display text-h2 font-[300] text-white/95">{t.admin.gallery.title}</h2>
        <p className="mb-6 font-sans text-caption text-mist">{t.admin.gallery.subtitle}</p>

        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          {SCENE_IDS.map((sceneId) => (
            <SceneCard key={sceneId} sceneId={sceneId} label={t.scenes[sceneId]} />
          ))}
        </div>
      </div>
    </div>
